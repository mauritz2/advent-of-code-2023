...
with open("day-14-data.txt") as f:
    ...
    for row_num in range(len(data)):
        ...
        for square in square_stones:
            blocks_by_col[square[1]].append(square[0])
            blocks_by_row[square[0]].append(square[1])
...
def tilt_board(round_stones, blocks, direction):
    new_round_rocks = []

    for round_stone in round_stones:
        blocks_on_col = blocks[round_stone[1]]
        nearest_blocks = [square for square in blocks_on_col if dir_mod[direction]["comp"](square, round_stone[0])]
        ...
        blocks[round_stone[1]].append(nearest_free_space)
        new_round_rocks.append(new_pos)

    return new_round_rocks

new_round_rocks = round_stones

for _ in range(1000000000):
    new_round_rocks = tilt_board(new_round_rocks, blocks_by_col, direction="North")
